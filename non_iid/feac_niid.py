...
def non_iid_partition(dataset, num_clients, num_shards=400, shard_size=150):
    # Convert dataset targets to numpy
    targets = dataset.targets.numpy()
    # Sorted indices based on label
    sorted_indices = np.argsort(targets)
    # Create shards
    shards = [sorted_indices[i * shard_size: (i + 1) * shard_size] for i in range(num_shards)]
    # Shuffle shards and assign 2 shards per client
...
def check_data_distribution(client_datasets, num_clients):
    class_counts = {i: collections.Counter() for i in range(num_clients)}
    for client_id, dataset in enumerate(client_datasets):
        # Extract labels
        labels = [dataset[i][1] for i in range(len(dataset))]  
        class_counts[client_id] = collections.Counter(labels)
    for client_id, counter in class_counts.items():
        print(f"Client {client_id}: {dict(counter)}") 
...
client_datasets = non_iid_partition(dataset, NUM_CLIENTS, num_shards=400, shard_size=150)
check_data_distribution(client_datasets, NUM_CLIENTS)
...
plt.tight_layout()
# Save
plt.savefig("obda_ea_dl_niid.png")  
print("Plot saved as obda_ea_dl_niid.png")
...
df.to_excel("obda_ea_dl_niid.xlsx", index=False)

print("Results saved to obda_ea_dl_niid.xlsx")